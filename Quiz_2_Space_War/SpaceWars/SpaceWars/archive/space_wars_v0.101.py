...
def message_display_left(text, font, color, centerX, centerY):
    text_surface, text_rectangle = text_objects(text, font, color)
    text_rectangle.center = (centerX,centerY)
    screen.blit(text_surface, text_rectangle)
...
#	global db_connection
	...
	while high_scores_screen:
		...
		message_display('High Scores', font_large, yellow, int(screen_sizeX/2), int(screen_sizeY * 1/10))
		message_display('Press (D)elete or any other key to continue', font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY *9/10))
		...
		for entry in top_5:
			...
			index += 1
			message_display(str(entry[1]), font_medium, yellow, int(screen_sizeX * 1/4), int(screen_sizeY *(2+index)/10))
			message_display(str(entry[2]), font_medium, yellow, int(screen_sizeX * 2/4), int(screen_sizeY *(2+index)/10))
			message_display(str(entry[3]), font_medium, yellow, int(screen_sizeX * 3/4), int(screen_sizeY *(2+index)/10))
		...
		for event in pygame.event.get():	
			if event.type == pygame.QUIT:
				...
				if event.key == pygame.K_d:
					print('D pressed to database will be deleted')
...
def menu():
	...
	while intro_screen:
		...
		message_display('SPACE WARS', font_large, yellow, int(screen_sizeX/2), int(screen_sizeY/3))
...
def show_game_over(screen_sizeX, screen_sizeY, score, high_score):
	...
	message_display('GAME OVER', font_large, yellow, int(screen_sizeX/2), int(screen_sizeY * 3/10))
	message_display('Your Score             : ' + str(score), font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY *5/10))
	message_display('Session Top Score : ' + str(high_score), font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY *6/10))
	message_display('Press any key to continue', font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY *8/10))
...
#			score += level_score_increase
	...
	if score > 0:
		high_scores.high_scores_update_db(db_connection, 'Dan', score)
