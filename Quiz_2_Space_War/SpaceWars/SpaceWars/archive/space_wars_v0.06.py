...
# One of two functions that are not yet used
...
#		play_again_font = pygame.font.Font('freesansbold.ttf', 32)
		...
		new_game_text = font_large.render('SPACE WARS', True, yellow)
		play_again_text = font_medium.render('New Game (Y/N)', True, yellow)
		...
		message_display('New Game (Y/N)', font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY *2/3))
		...
		# Define button1 (and button) 
		button1_X = 150
		button1_Y = 450
		button1_width = 100
		button1_hight = 50
		pygame.draw.rect(screen, yellow, (button1_X, button1_Y, button1_width, button1_hight))
		if button1_X < mouse[0] < button1_X + button1_width and button1_Y < mouse[1] < button1_Y + button1_hight:
			pygame.draw.rect(screen, light_yellow, (button1_X, button1_Y, button1_width, button1_hight))
		else:
			pygame.draw.rect(screen, yellow, (button1_X, button1_Y, button1_width, button1_hight))
...
enemy_image	    	= [pygame.image.load(os.path.join(images_path, 'ufo_01.png')),
				       pygame.image.load(os.path.join(images_path, 'death_star_01.png')),
...
while not quit_game:
	...
	#	enemy_image_index = random.randint(1,len(enemy_image)) - 1
	...
	while not go_to_menu and not quit_game:
		...
		if level_iter > level_change and not game_over:
			...
			for i in range(num_of_enemies, num_of_enemies+level_enemy_increase):
				enemy_image_index = level % len(enemy_image)
				#	enemy_image_index = random.randint(1,len(enemy_image) - 1)
				enemy.append(SpaceEnemy(enemy_image[enemy_image_index], explosion_image[1], speedY = level, hit_points = level))
				enemy_respawn(enemy[i], level)
